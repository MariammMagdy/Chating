    ...
    check("email")
        ...
        .custom(async (val, { req }) => {
            ...
            if (!gmailRegex.test(val)) {
                throw new Error(
                "email must be starting with char, Matches the '@' symbol and Non-capturing group that matches the literal characters 'gmail.com '"
                );
            }
        }),
    ...
    check("newPassword")
        ...
        .custom((newPassword, { req }) => {
            ...
            if (!strongPasswordRegex.test(newPassword)) {
                throw new Error(
                "Password must be at least 8 characters ,having at least one capital letter ,having at least one small letters ,having at least one digit and one special charachter"
                );
            }
        return true;
        }),
